...
export const subscaleCodeEnum = pgEnum('subscale_code', [
  'MD',
  'PD',
  'TD',
  'OP',
...
export const phaseOrderEnum = pgEnum('phase_order', [
  'comparisons_first',
  'ratings_first',
])
...
export const participants = pgTable(
  ...
  (t) => [
    uniqueIndex('participants_study_code_unique').on(
      t.studyId,
      t.participantCode,
    ),
  ],
...
export const sessions = pgTable('sessions', {
  ...
  phaseOrder: phaseOrderEnum('phase_order')
    .notNull()
    .default('comparisons_first'),
...
export const pairwiseComparisons = pgTable(
  ...
  (t) => [
    ...
    check(
      'pair_index_range',
      sql`${t.pairIndex} >= 0 AND ${t.pairIndex} <= 14`,
    ),
  ],
)

export const subscaleRatings = pgTable(
  'subscale_ratings',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    sessionId: uuid('session_id')
    ...
    sliderPosition: decimal('slider_position', {
      precision: 5,
      scale: 2,
    }).notNull(),
    respondedAt: timestamp('responded_at').defaultNow().notNull(),
